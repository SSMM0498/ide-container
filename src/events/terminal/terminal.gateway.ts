...
export class TerminalGateway {
    ...
    async startPreview(@MessageBody() eventData: string) {
        try {
            ...
            configParser.read(`${this.codeDirectory}/rdamn.cfg`)
    ...
    initConnexion(@ConnectedSocket() client: Socket): Observable<WsResponse<string>> {
        ...
        return new Observable((observer: Observer<WsResponse<string>>) => {
            ...
            client.emit('terminal-data', {
                data: `${bright}${red}${rdamnLogo}${reset}${EOL}`,
            });

            client.emit('terminal-data', {
                data: `${BgWhite}${bright}${red}Connected! Welcome to rdamn!${reset}${EOL}${EOL}`
            });

            client.emit('terminal-data', { data: `${bright}${green}rdamn@${this.hostname.split(".")[0]}${reset}:${bright}${blue}~/code${reset}$ ` });
